...
def register_handlers(bot: commands.Bot):
    ...
    async def on_message(message: Message):
        ...
        try:
            ...
            response = "Zapisałeś się do mojego newslettera. Będę Cię informował wszystkich błędach wywołanych przez Twoją niekompetencję!"
            if message.channel in SET:
                response = "Kanał już został zasubskrybowany"
            else: 
                response = "Zapisałeś się do mojego newslettera. Będę Cię informował wszystkich błędach wywołanych przez Twoją niekompetencję!"
            ...
            if len(response) <= MAX_LENGTH:
                ...
                await message.channel.send(chunks[0])  # only the first chunk as reply
